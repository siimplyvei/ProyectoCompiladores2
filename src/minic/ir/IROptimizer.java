    ...
    public static List<IRInstr> deadCodeElimination(List<IRInstr> ir) {
        ...
        while (it.hasPrevious()) {
            ...
            if (instr instanceof IRBinOp bin) {
                ...
                if (instr instanceof IRCall || instr instanceof IRStore) {
                    result.add(instr);
                    continue;
                }
                ...
                // Un load que alimenta una condición NO puede eliminarse
                if (!live.contains(dst)) {
                    continue;
                }
                live.remove(dst);
                ...
                // TODAS las llamadas tienen efectos secundarios
                if (call.result != null) {
                ...
                result.add(instr);
            } else if (instr instanceof IRParam p) {
                // IRParam USA su valor → debe marcarse como vivo
                live.add(p.value);
                result.add(instr);
            } else {
    ...
    public static List<IRInstr> removeConstantConditions(List<IRInstr> ir) {
        ...
        for (int i = 0; i < ir.size(); i++) {
            ...
            if (instr instanceof IRIfZ ifz) {
                // Solo eliminar si es un literal constante NO VARIABLE
                if (ifz.condition.matches("\\d+") && !ifz.condition.startsWith("t")) {
